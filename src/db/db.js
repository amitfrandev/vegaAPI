...
async function saveMovie(movie, options = {}) {
  return new Promise((resolve, reject) => {
    try {
      ...
      db.get(`SELECT * FROM movies WHERE ${urlMatchCondition}`, [normalizedUrl], async (err, row) => {
          if (err) {
        ...
        if (row) {
          ...
          const tagsChanged = JSON.stringify(existingTags.sort()) !== JSON.stringify(movie.tags.sort());
          ...
          if (options.forceUpdate || tagsChanged || options.forceTagUpdate) {
            ...
            if (options.forceUpdate) {
              db.run(
                ...
                function(err) {
                  ...
                  console.log(`Fully updated movie: ${movie.title} (${movie.url})`);
            ...
            if (err) {
                  ...
                  console.log(`Tags changed: ${existingTags.join(', ')} -> ${movie.tags.join(', ')}`);
                  resolve({exists: true, updated: true});
                }
              );
            }
          } else {
            console.log(`Skipped update for movie: ${movie.title} (tags unchanged)`);
          ...
          db.run(
            ...
            function(err) {
              ...
              console.log(`Inserted new movie: ${movie.title} (${movie.url})`);
...
async function getMovieByUrl(url) {
  return new Promise((resolve, reject) => {
    try {
      ...
      db.get(`SELECT * FROM movies WHERE ${urlMatchCondition}`, [normalizedUrl], (err, movieRow) => {
      if (err) {
...
async function getMovieById(id) {
  return new Promise((resolve, reject) => {
    db.get(`SELECT * FROM movies WHERE id = ?`, [id], (err, movieRow) => {
      ...
      // Get screenshots
      db.all(`SELECT url FROM screenshots WHERE movie_id = ?`, [movieRow.id], (err, screenshotRows) => {
        if (err) {
        ...
        // Get sections
        db.all(`SELECT * FROM sections WHERE movie_id = ?`, [movieRow.id], (err, sectionRows) => {
          if (err) {
            console.error('Error getting sections:', err.message);
            return reject(err);
          }
          
          console.log(`Found ${sectionRows.length} sections for movie ${movieRow.title}`);
          
          const sectionPromises = sectionRows.map(sectionRow => {
            return new Promise((resolve, reject) => {
              // Get link groups for this section
              db.all(`SELECT * FROM link_groups WHERE section_id = ?`, [sectionRow.id], (err, linkGroupRows) => {
                if (err) {
                  console.error('Error getting link groups:', err.message);
                  return reject(err);
                }
                
                console.log(`Found ${linkGroupRows.length} link groups for section "${sectionRow.heading || sectionRow.note}"`);
                
                const linkGroupPromises = linkGroupRows.map(linkGroupRow => {
                  return new Promise((resolve, reject) => {
                    // Get links for this link group
                    db.all(`SELECT button_label, url as link FROM links WHERE link_group_id = ?`, [linkGroupRow.id], (err, linkRows) => {
                      if (err) {
                        console.error('Error getting links:', err.message);
                        return reject(err);
                      }
                      
                      // Log for debugging
                      console.log(`Found ${linkRows.length} links for group "${linkGroupRow.name}"`);
            ...
            .then(sections => {
              ...
              const movie = {
                ...
                info: [{
                  ...
                  details: movieRow.details ? JSON.parse(movieRow.details) : [],
...
async function getAllMovies(page = 1, pageSize = 20, options = {}) {
  ...
  try {
    ...
    return { movies: [], pagination: { page, pageSize, totalPages: 0, totalItems: 0, hasNextPage: false, hasPrevPage: false } };
  }
}
      ...
      db.all(query, paginatedParams, (err, rows) => {
        ...
        const movies = rows.map(row => {
          try {
            ...
            console.error(`Error parsing movie info for ${row.title}:`, parseError);
            return {
              id: row.id,
              title: row.title,
              url: row.url,
...
async function getMovieStats() {
  try {
    ...
    const uniqueMovies = await new Promise((resolve, reject) => {
      db.get("SELECT COUNT(*) as count FROM movies WHERE json_extract(info, '$[0].movie_or_series') = 'Movie'", (err, row) => {
        if (err) reject(err);
        else resolve(row.count);
      });
    });
    ...
    const uniqueSeries = await new Promise((resolve, reject) => {
      db.get("SELECT COUNT(*) as count FROM movies WHERE json_extract(info, '$[0].movie_or_series') = 'TV Series' OR json_extract(info, '$[0].movie_or_series') = 'Series'", (err, row) => {
        if (err) reject(err);
        else resolve(row.count);
      });
    });
...
async function searchMoviesByTags(tags) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const whereConditions = tagJsonStrings.map(tagJson => 
...
async function saveCategorySitemap(categoryData) {
  try {
    ...
    stmt.finalize();
    console.log(`Saved ${Object.keys(categoryData.categories).length} category types to database`);
    return true;
  } catch (error) {
...
async function getCategorySitemap() {
  try {
    ...
    console.error('Error retrieving category sitemap from database:', error.message);
    return null;
  }
}
