...
async function loadProcessedMovies() {
  try {
    ...
    if (config.movieChunking && config.movieChunking.enabled) {
      ...
      console.log(
        `Loaded ${allMovies.length} movies from ${indexData.chunks.length} chunks`
      );
      return allMovies;
    } else {
      console.log(
        "Movie chunking is disabled but we only support chunked mode now"
      );
      return [];
    }
  } catch (error) {
    ...
    if (sections && sections.length > 0) {
      for (
        ...
        if (section.links && section.links.length > 0) {
          ...
          for (
            let groupIndex = 0;
            groupIndex < section.links.length;
            groupIndex++
          ) {
            const group = section.links[groupIndex];

            console.log(
              `Processing button type: ${group.type} (${group.buttonLabel})`
            );
            ...
            if (
              ...
              if (group.type === "Batch/Zip") {
                console.log(
                  "Processing Batch/Zip links to extract actual download links"
                );

                // Collect size information from the button label
                let zipSize = "";
                const sizeMatch =
                  group.buttonLabel.match(/\[([0-9.]+[MG]B)\]/i);
                if (sizeMatch) {
                  zipSize = sizeMatch[1];
                }
                ...
                if (section.heading) {
                  const qualityMatch =
                    section.heading.match(/(480p|720p|1080p)/i);
                  if (qualityMatch) {
                ...
                for (const [episodeNum, nexdriveUrl] of Object.entries(
                  group.links
                )) {
                  totalLinksFound++;
                  ...
                  if (nexdriveUrl.includes("nexdrive")) {
                    ...
                    try {
                      ...
                      const downloadLinks = await fetchNextdriveLinks(
                        nexdriveUrl,
                        `Batch Zip ${zipSize}`
                      );
                      ...
                      if (downloadLinks && downloadLinks.length > 0) {
                        console.log(
                          `Found ${downloadLinks.length} direct links from Batch/Zip nexdrive page`
                        );
                        ...
                        downloadLinks.forEach((buttonTypeData) => {
                          ...
                          if (buttonTypeData.link) {
                            ...
                            buttonTypeData.links &&
                            Object.keys(buttonTypeData.links).length > 0
                          ) {
                            // Episode-based link format - extract each link
                            for (const [linkEpisode, linkUrl] of Object.entries(
                              buttonTypeData.links
                            )) {
                              batchZipLinks.links.push({
                      ...
                      console.error(
                        `Error processing Batch/Zip nexdrive link: ${error.message}`
                      );
                ...
                for (const [episodeNum, batchLinks] of Object.entries(
                  batchLinksByEpisode
                )) {
              ...
              if (
                ...
                for (const [episodeNum, nexdriveUrl] of Object.entries(
                  ...
                  if (
                    ...
                    if (!episodeLinksByType[group.type]) {
                      episodeLinksByType[group.type] = {
                        buttonLabel:
                          group.buttonLabel || `${group.type} Button`,
                        type: group.type,
                        links: {},
                      };
                    }
                    episodeLinksByType[group.type].links[episodeNum] =
                      nexdriveUrl;
                    console.log(
                      `Added non-nexdrive link for episode ${episodeNum}: ${nexdriveUrl}`
                    );
                    continue;
                  }
                  ...
                  try {
                    ...
                    if (downloadLinks && downloadLinks.length > 0) {
                      console.log(
                        `Found ${downloadLinks.length} direct link types for episode ${episodeNum}`
                      );
                      ...
                      downloadLinks.forEach((buttonTypeData) => {
                        ...
                        if (
                          ...
                          if (Object.keys(buttonTypeData.links).length === 1) {
                            const firstLink = Object.values(
                              buttonTypeData.links
                            )[0];
                            episodeLinksByType[buttonType].links[episodeNum] =
                              firstLink;
                            console.log(
                              `Added ${buttonType} direct link for episode ${episodeNum}: ${firstLink}`
                            );
                          } else {
                            ...
                            for (const [
                              linkEpisode,
                              directUrl,
                            ] of Object.entries(buttonTypeData.links)) {
                              const finalEpisode =
                                linkEpisode === "1" && episodeNum !== "1"
                                  ? episodeNum
                                  : linkEpisode;
                              episodeLinksByType[buttonType].links[
                                finalEpisode
                              ] = directUrl;
                              console.log(
                                `Added ${buttonType} direct link for episode ${finalEpisode}: ${directUrl}`
                              );
                            }
                          }
                        }
                      ...
                      episodeLinksByType[group.type].links[episodeNum] =
                        nexdriveUrl;
                      console.log(
                        `No direct links found, keeping original nexdrive link for episode ${episodeNum}`
                      );
                    }
                  } catch (error) {
                    console.error(
                      `Error processing nexdrive link: ${error.message}`
                    );
                ...
                processedSectionLinks.push(
                  ...Object.values(episodeLinksByType)
                );
              } else {
...
function detectEpisodeBasedContent(mainContent, $) {
  ...
  mainContent.find("h2, h3, h4, h5").each(function () {
    ...
    if (
      headingText.match(/Season\s+\d+/i) &&
...
function processEpisodeBasedLinks(mainContent, $) {
  ...
  mainContent.find("h3").each(function () {
    ...
    if (
      ...
      let size = "";
      const sizeMatch =
        headingText.match(/\[([0-9.]+[MG]B)\/E\]/i) ||
        ...
        if (nextElement.is("p")) {
          nextElement.find("a").each(function () {
            ...
            if (href && !href.includes("#")) {
              ...
              const episodeMatch =
                parentText.match(/Episode[s]*\s*(\d+)/i) ||
              ...
              console.log(
                `Added ${buttonType} link for episode ${episodeNum}: ${href}`
              );
            }
          });
        }
      ...
      if (Object.keys(linksByButtonType).length > 0) {
        ...
        console.log(
          `Added section "${headingText}" with ${section.links.length} button types`
        );
      }
    }
  });
  ...
  if (sections.length === 0) {
    console.log(
      "No episode-based sections found, falling back to quality-based structure"
    );
...
async function processQualityBasedLinks(mainContent, $) {
    ...
    if (
      ...
      if (nextP.length > 0) {
        ...
        if (links.length > 0) {
          ...
          console.log(
            `Added ${links.length} links to group "${headingText}" with quality: ${headingQuality}, size: ${headingSize}`
          );
        }
      }
    }
  ...
  if (Object.keys(qualityGroups).length > 0) {
    ...
    for (const [heading, groupData] of Object.entries(qualityGroups)) {
      ...
      sections.push(section);
      console.log(
        `Created section with heading: "${heading}" and name: "${linkName}", quality: ${groupData.quality}, size: ${groupData.size}`
      );
    }
  } else {
      ...
      if (nextP.length > 0) {
        nextP.find('a[href*="nexdrive"]').each(function () {
          ...
          if (
            ...
            console.log(
              `Found nexdrive link: ${buttonText} - ${href} with heading: ${headingText}`
            );
            linksFound++;
    ...
    if (Object.keys(headingMap).length > 0) {
      for (const [heading, data] of Object.entries(headingMap)) {
        ...
        console.log(
          `Created section with heading: "${heading}" and name: "${linkName}", quality: ${data.quality}, size: ${data.size}`
        );
      }
    }
    // Fall back to direct links if headingMap is empty
      ...
      console.log(
        `Created section with direct links: ${directLinks.length}, quality: ${headingQuality}, size: ${headingSize}`
      );
    }
  }
  ...
  for (const section of originalSections) {
    ...
    for (const linkGroup of section.links) {
      ...
      if (
        (linkGroup.name && linkGroup.name.includes("Batch/Zip")) ||
        (linkGroup.links &&
          linkGroup.links.some(
            (link) => link.buttonLabel && link.buttonLabel.includes("Batch/Zip")
          ))
      ) {
              ...
              if (downloadLinks && downloadLinks.length > 0) {
                ...
                for (const dlLink of downloadLinks) {
                  if (dlLink.link) {
                    ...
                    console.log(
                      `Added extracted link: ${dlLink.buttonLabel} (${dlLink.type})`
                    );
                  } else if (
                    dlLink.links &&
                    Object.keys(dlLink.links).length > 0
                  ) {
                    // Handle episode-specific links format (though rare in movie content)
                    for (const [episodeNum, episodeLink] of Object.entries(
                      dlLink.links
                    )) {
                      batchZipLinks.links.push({
                        buttonLabel: `${
                          dlLink.buttonLabel || dlLink.type
                        } [Episode ${episodeNum}]`,
                        link: episodeLink,
                        type: dlLink.type,
                      });
                      console.log(
                        `Added extracted episode link: ${dlLink.buttonLabel} [Episode ${episodeNum}]`
                      );
                    }
                  }
                }
                ...
                console.log(
                  `No download links found for ${link.link}, keeping original`
                );
      ...
      // Check if we have any nexdrive links to process (non Batch/Zip)
        ...
        console.log(
          `Found nexdrive links in group "${linkGroup.name}", processing...`
        );
        ...
        for (const link of linkGroup.links) {
          ...
          try {
            ...
            if (downloadLinks && downloadLinks.length > 0) {
              console.log(
                `Found ${downloadLinks.length} direct links from nexdrive page`
              );
      ...
      if (
        processedLinkGroup.links.length > 0 &&
        !(linkGroup.name && linkGroup.name.includes("Batch/Zip"))
      ) {
...
function extractImdbRating($) {
  ...
  if (imdbElement.length) {
    for (let i = 0; i < imdbElement.length; i++) {
      ...
      const imdbMatch =
        imdbText.match(/IMDb Rating\s*:?-?\s*([0-9.]+\/10)/i) ||
...
function extractSynopsis($) {
  ...
  const synopsisHeading = $("h2, h3, h4").filter((i, el) => {
    return (
      $(el).text().toLowerCase().includes("synopsis") ||
...
async function getMovieList(page = 1) {
  try {
    ...
    console.log(
      `${bgMagenta}${white}${bright} 🎬  Fetching movie list from: ${url} ${reset}`
    );
    console.log(`${magenta}${bright}==============================${reset}`);
    ...
    if (shouldReverseOrder) {
      ...
      console.log(
        `Found ${movieElements.length} movies on page ${page}, processing in reverse order (last to first)`
      );
    } else {
      console.log(
        `Found ${movieElements.length} movies on page ${page}, processing in order (first to last)`
      );
    }
...
async function parseMovieInfo($, movie, options = {}) {
  try {
    ...
    if (existingMovie && !options.forceUpdate) {
      console.log(
        `Movie "${movie.title}" already exists in database, skipping nexdrive processing completely`
      );
    ...
    const info = {
      ...
      title:
        contentType === "series" ? extractedSeriesName : extractedMovieName,
    ...
    try {
                ...
                group.links.forEach((link) => {
                  ...
                  if (link && link.link) {
                    ...
                    if (link.link.includes("fastdl.icu")) {
                      ...
                      link.link.includes("filebee.xyz") ||
                      link.link.includes("filepress.icu")
                    ) {
                      filepressLinks++;
                    } else if (
                      link.link.includes("dgdrive.pro") ||
                      link.link.includes("dgdrive.site")
                    ) {
                      dropgalaxyLinks++;
                    } else {
                      otherLinks++;
      ...
      const fallbackSections =
        (await processFallbackLinks($, $("main").html())) || [];
...
async function getMovieDetails(movie, options = {}) {
  try {
    ...
    if (existingMovie && !options.forceUpdate) {
      console.log(
        `Movie "${movie.title}" already exists in database, completely skipping network request`
      );
    ...
    const $ = cheerio.load(content);
    const movieInfo = await parseMovieInfo($, movie, {
    ...
    if (movieInfo === null) {
      ...
      if (existingMovie) {
        console.log(
          `Movie "${movie.title}" found during content parsing, returning existing record`
        );
        return existingMovie;
      }
    ...
    if (movieInfo) {
      ...
      try {
        ...
        await db.saveMovie(movieWithDetails, {
          forceUpdate: options.forceUpdate,
        });
        return movieWithDetails;
      } catch (error) {
        console.error(
          `Error saving movie "${movie.title}" to database:`,
          error
        );
...
async function parseMovieInfoWithoutNexdrive($, movie) {
  try {
    ...
    if (existingMovie) {
      console.log(
        `Movie "${movie.title}" already exists in database, skipping content parsing`
      );
      return null; // Return null to signal movie exists but shouldn't be processed
    }
...
async function fetchNextdriveLinks(url, heading = null) {
  ...
  try {
    ...
    try {
      const pageName = heading
        ? `nexdrive_${heading.replace(/\s+/g, "_")}`
        : null;
    ...
    $(
      ".entry.themeform h1, .entry.themeform h2, .entry.themeform h3, .entry.themeform h4, .entry.themeform h5"
    ).each(function () {
      const headingText = $(this).text().trim();
      ...
      if (episodeMatch) {
        ...
        console.log(
          `Found episode heading: ${headingText} (Episode ${episodeNumber})`
        );
      }
    });
    ...
    const hasEpisodeText =
      /episode\s*\d+/i.test(pageContent) ||
    ...
    if (heading) {
      const episodeMatch =
        heading.match(/Episode[s]*[:\s-]+(\d+)/i) || heading.match(/E(\d+)/i);
      if (episodeMatch) {
    ...
    if (episodeHeadings.length > 0) {
      console.log(
        `Found ${episodeHeadings.length} episode headings in content`
      );
        ...
        while ((currentEl = currentEl.next()).length > 0) {
          ...
          currentEl.find("a").each(function () {
            ...
            if (href && !href.includes("#") && href !== url) {
              ...
              const button = link.find("button");
              const buttonText =
                button.length > 0 ? button.text().trim() : link.text().trim();
              const buttonType = getButtonType(buttonText);
              ...
              console.log(
                `Found direct download link for Episode ${episodeNum}: ${
        ...
        if (!foundLinks) {
          ...
          for (const [buttonType, buttons] of Object.entries(buttonGroups)) {
            ...
            if (buttons.length === episodeHeadings.length) {
              const idx = episodeHeadings.findIndex(
              ...
              if (idx >= 0 && idx < buttons.length) {
                ...
                console.log(
                  `Found direct download link for Episode ${episodeNum} (by matching count): ${
            ...
            if (href && !href.includes("#") && href !== url) {
              ...
              if (!buttonTypes[buttonType]) {
                ...
                if (headingEpisodeNum || hasEpisodeText) {
                  ...
                  buttonTypes[buttonType].links[headingEpisodeNum || "1"] =
                    href;
                  console.log(
                    `Found ${buttonType} link for Episode ${
                      headingEpisodeNum || "1"
                    }: ${buttonText || "Download Now"} -> ${href}`
                  );
                } else {
                  // For non-episode content, use direct link
                  buttonTypes[buttonType].link = href;
                  console.log(
                    `Found ${buttonType} link (non-episode): ${
                      buttonText || "Download Now"
                    } -> ${href}`
                  );
                }
                ...
                if (buttonTypes[buttonType].links) {
                  ...
                  if (headingEpisodeNum || hasEpisodeText) {
                    buttonTypes[buttonType].links[headingEpisodeNum || "1"] =
                      href;
                    console.log(
                      `Found additional ${buttonType} link for Episode ${
                        headingEpisodeNum || "1"
                      }: ${buttonText || "Download Now"} -> ${href}`
                    );
                  }
                  ...
                  console.log(
                    `Additional ${buttonType} link found but using first one: ${
                      buttonText || "Download Now"
                    } -> ${href}`
                  );
                }
      ...
      if (standardResults.length === 0) {
        ...
        $(".btn.btn-sm.btn-outline, .sml-button").each(function () {
          ...
          if (parentLink.length > 0) {
            ...
            if (href && !href.includes("#") && href !== url) {
              ...
              console.log(
                `Found direct download button: ${buttonText} -> ${href}`
              );
            }
          }
        });
          ...
          $(this)
            ...
            .each(function () {
              ...
              if (href && !href.includes("#") && href !== url) {
                ...
                if (
                  alertText.includes("Fast Server") ||
                  alertText.includes("V-Cloud")
                ) {
                  type = "V-Cloud";
                ...
                console.log(
                  `Found link in alert: ${linkText || type} -> ${href}`
                );
              }
            });
        });
        ...
        if (extractedLinks.length > 0) {
          console.log(
            `Found ${extractedLinks.length} links through fallback extraction`
          );
